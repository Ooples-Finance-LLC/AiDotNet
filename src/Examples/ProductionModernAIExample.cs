...
namespace AiDotNet.Examples
{
    ...
    public static class ProductionModernAIExample
    {
        ...
        public static async Task InterpretabilityExampleAsync()
        {
            ...
            // Wrap in explainable model
            var explainableModel = new ExplainableModelWrapper<double>(model);
            
            // Get feature importance
            var globalImportance = explainableModel.GetFeatureImportance();
            Console.WriteLine("Global Feature Importance:");
            foreach (var (feature, importance) in globalImportance)
            {
                Console.WriteLine($"  Feature {feature}: {importance:F4}");
            }
            
            // Get local explanation for a single prediction
            var sampleInput = new Vector<double>(features[0]);
            var sampleTensor = new Tensor<double>(sampleInput.ToArray());
            var explanation = explainableModel.ExplainPrediction(sampleTensor);
            
            Console.WriteLine("\nLocal Explanation for Sample:");
            Console.WriteLine($"  Prediction: {explanation.Prediction:F4}");
            Console.WriteLine($"  Confidence: {explanation.Confidence:F4}");
            Console.WriteLine("  Feature Contributions:");
            foreach (var (feature, contribution) in explanation.FeatureContributions)
            {
                Console.WriteLine($"    Feature {feature}: {contribution:F4}");
            }
        }
