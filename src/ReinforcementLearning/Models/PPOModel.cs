...
{
    /// <summary>
    /// Reinforcement Learning model implementing the Proximal Policy Optimization (PPO) algorithm.
    /// </summary>
    ...
    public class PPOModel<T> : ReinforcementLearningModelBase<T>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PPOModel{T}"/> class.
        /// </summary>
        /// <param name="options">The options for the PPO algorithm.</param>
        public PPOModel(PPOOptions<T> options)
            : base(options)
        {
            _options = options;
        ...
        protected override void InitializeAgent()
        {
            if (IsContinuous)
            {
                _continuousAgent = new PPOAgent<Tensor<T>, Vector<T>, T>(_options);
                _discreteAgent = null;
            }
            else
            {
                _discreteAgent = new PPOAgent<Tensor<T>, int, T>(_options);
        ...
        public override ModelMetadata<T> GetModelMetadata()
        {
            var metadata = new ModelMetadata<T>
            {
                ...
                AdditionalInfo = new Dictionary<string, object>
                {
                    ...
                    { "ClipParameter", _options.ClipParameter },
                    { "UseValueClipping", _options.UseValueClipping },
                    { "UseKLPenalty", _options.UseKLPenalty }
                }
            };
        ...
        {
            return new PPOModel<T>(_options);
        }
        /// <summary>
        ...
        public override void Save(Stream stream)
        {
            using (var writer = new BinaryWriter(stream, System.Text.Encoding.UTF8, true))
            {
                ...
                writer.Write(_options.ClipParameter);
                writer.Write(_options.UseValueClipping);
                writer.Write(_options.UseKLPenalty);
        ...
        public override void Load(Stream stream)
        {
            using (var reader = new BinaryReader(stream, System.Text.Encoding.UTF8, true))
            {
                ...
                if (modelType != "PPOModel")
                {
                    throw new InvalidOperationException($"Expected PPOModel, but got {modelType}");
                }
                ...
                double clipParameter = reader.ReadDouble();
                bool useValueClipping = reader.ReadBoolean();
                bool useKLPenalty = reader.ReadBoolean();
